...
{/*------------------------------ Trades  ------------------------------- */}
<button onClick={()=>setshowtrades(!showtrades)}>show</button>
...
useEffect(()=>{
  ...
  return (
    ...
    <div>
          ...
          <div>
          <h3 style={{fontSize:"1.125rem"}} >Trades <button  className='btn'>{trades?.length>0?<i className="bi bi-chevron-up"></i>:<i className="bi bi-chevron-down"></i>}</button> {trades?.length>0?<>({trades?.length})</>:null}</h3>
          </div> 

         

          {trades?.length>0?   <div className="d-flex align-items-baseline">
          <div>
          <div className="holdingsearch">
          <label className="m-1 mx-2 " ><i className="bi bi-search"></i></label>
          <input className="inputsearch" placeholder="Search" type="text" />
